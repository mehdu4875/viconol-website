...
export default function Footer({ locale }: { locale: string }) {
  ...
  return (
    <footer className="bg-[#0a0a0a] text-gray-400 border-t border-white/5 mt-auto">
          ...
          <ul className="space-y-4">
            {navLinks.map((link) => (
              <li key={link.href} className="flex justify-center md:justify-start">
                <Link href={link.href} className="text-sm font-semibold hover:text-white transition-colors uppercase tracking-widest flex items-center gap-2">
        ...
        <div className="flex flex-col gap-6 items-center md:items-start md:col-span-2">
          ...
          <ul className="space-y-6">
            <li className="flex gap-4 items-center">
              ...
              <div>
                <span className="font-bold text-white">{tContact('support_title')}</span>
                <p className="text-sm font-medium">info@viconol.com</p>
              </div>
            </li>
            <li className="flex gap-4 items-center">
              <Clock className="w-5 h-5 text-viconol-primary flex-shrink-0" />
              <div>
                <span className="font-bold text-white">{tContact('hours_title')}</span>
                <p className="text-sm font-medium">Mo - Fr: 8:00 - 18:00</p>
              </div>
            </li>
          </ul>
        </div>
      </div>
      <div className="bg-black border-t border-white/5 py-6">
